...
network.on("selectNode", function (params) {
  ...
  if (params.nodes.length == 1) {
    nodes_ordered = [chosen_node]
  } else {
    nodes_ordered.push(chosen_node)
  }
  console.log('selectNode - Nodes Selected: ' + nodes_ordered);
...
network.on("deselectNode", function (params) {
  ...
  if (params.nodes.length == 0) {
    ...
    chosen_node = params.nodes[0]
    nodes_ordered = [chosen_node]
  } else {
  chosen_node = this.getNodeAt(params.pointer.DOM)
  nodes_ordered = arrayRemove(nodes_ordered, chosen_node);
  }
  console.log('deselectNode - Nodes Selected: ' + nodes_ordered);
